...
experiment_name       = "_"
cell_type             = "Jurkat" # e.g. Jurkat, SU.DHL.6 Name has to match CSPA terminology (supplementary table)
...
string <- string %>%
  select(all_of(c(string_linkage_categs, c("protein1_entry", "protein2_entry", "protein1_entry_name", "protein2_entry_name", "reviewed_protein1", "meta_surfaceome_protein1", "combined_score")))) %>%  # filter out unwanted categories  ...
...
## Directory containing standardized list of glycosylated Peptides/Proteins found, serves as "own" CSPA
glyco_standard_directory  = c("/Volumes/mgesell/03_DataProcessing/3_PBMCs/Jurkat_CSPA")    # or c("/Volumes/mgesell/03_DataProcessing/3_PBMCs/primaryT8_CSPA")
Jurkat_CSPA               = read.csv(paste(glyco_standard_directory, "/Jurkat_CSPA.csv", sep = ""), row.names = 1)     # Load "own" CSPA from standard directory
...
if (data_type == "DDA") { 
  ...
  data_raw  <- data_raw_ %>% 
    ...
    select(-trash, -entry_name) %>%  # entry_name removed because needs be added for DIA & if present already messes up colnames to x. y. 
  ...
  file_annotation <- read_protti(paste(input_directory, "/msstats.csv", sep = "")) %>% 
    select(run, condition, bio_replicate) %>% 
  ...
  data_raw  <- data_raw_ %>% 
    select(run, all_mapped_proteins, protein_group, modified_sequence, stripped_sequence, precursor_charge, precursor_quantity, rt, rt_start, rt_stop) %>% #  rt_start, rt_stop
    filter(!(run %in% paste(filter_exclude_files))) %>% # Exclude files based on filter_exclude_files
...
data_raw_up <- data_raw_up %>% select(-aa_before, -last_aa, -aa_after)  # no longer needed after this step >> removed
...
write.csv(data_raw_up_surf_gly_nZ %>% filter(lux_signature == "yes") %>% select (condition, bio_replicate, con_rep, entry_name, protein_name, peptide_sequence_mod, sequence_veneer, cd_antigen, uniprot_2023, tcsa_2021, surfy_2018, cspa_2015, meta_surfaceome)
...
glycoAnnotDf   <- glycoAnnotDf %>% select(Category, no, yes)
...
qc_plot_surface_1 <-  data_raw_up_surf_gly_nZ %>% 
  select(con_rep, entry, !!qc_reference) %>%
  unique() %>%
...
qc_plot_surface_2 <-  data_raw_up_surf_gly_nZ %>% 
  select(con_rep, entry, plot_signature) %>%
  unique() %>%
...
qc_plot_surface_6 <-  data_raw_up_surf_gly_nZ %>%   
  select(con_rep, peptide_sequence_mod, !!sym(qc_reference)) %>%
  unique() %>%
  ggplot(aes_string("con_rep", fill = qc_reference))+
  geom_bar(color = "black", position = "dodge") +
  theme_classic()+
...
qc_plot_surface_7 <-  data_raw_up_surf_gly_nZ %>% 
  select(con_rep, peptide_sequence_mod, plot_signature) %>%
  unique() %>%
  ggplot(aes(con_rep, fill = plot_signature))+
  geom_bar(color = "black", position = "dodge") +
  theme_classic()+
...
qc_plot_surface_9 <- data_raw_up_surf_gly_nZ[data_raw_up_surf_gly_nZ$plot_signature != "cont",] %>% 
  select(con_rep, plot_signature, raw_prec_intensity) %>% mutate(plot_signature = as.character(plot_signature)) %>%
...
## clean up dataframe
data_raw_pre_filtered <- data_raw_pre_filtered %>% select( -sequence_extended, -sequence_veneer)
...
if (data_processing == "protti"){    #-------- Protti -------- Protti -------- Protti -------- Protti -------- Protti -------- Protti -------- Protti -------- Protti 
  ...
  data_prec_norm_feature_filtered <- data_prec_norm_feature_filtered %>%
    ...
    select(-sd_precursor, -mean_precursor) %>%
    ungroup()  %>%
  ...
  for (i in 1:length(experiment_references)) {
    ...
    data_prot_missing <- data_prot_missing %>% 
      ...
      select(-features_right_dummy, -replicate_ids_right_dummy) %>% # ***remove dummy column
      mutate(
  ...
  data_prot_mis_filtered_imp <- data_prot_mis_filtered_imp %>%
    ...
    # select(-sd_protein, -mean_protein) %>%
    ungroup()  %>%
    mutate(meta_surfaceome = ifelse(entry %in% surface_annotations$cspa_2015surfy_2018tcsa_2021cd_antigen_veneer_proteome_high, "yes", "no")) %>% # for visualization
  ...
  cv_protein_merge_helper_df <- data_prot_mis_filtered_imp %>%
      select(entry, condition, comparison, cv_protein) %>%
      separate(comparison,   into = c("ttest_condition", "ttest_reference"), 
               sep = "_vs_", remove = FALSE) %>%
    ...
    select(-ttest_condition, -ttest_reference)
  ...
  # assign imputation 
  data_prot_diff_abundance <- data_prot_diff_abundance %>%
    select(-imputed) %>%   # when protein df is collapsed to comparison df the imputation column looses its value (retained but does not make sense anymore)
    group_by(comparison) %>% 
...
rm("current_plot", "data_CSPA_fraction", "data_reps", "expected_proteins", "qc_precursor_cv", "qc_precursor_cv_i", "qc_prot_cv", "qc_prot_cv_i",              # remove stuff
   ...
   "diff_calc_input", "glycoAnnotDf", "glycoAnnotDf_L", "grid_plot", "grouped_data", "Jurkat_CSPA", "data_raw_pre_filtered", "data_raw_up_surf_gly",          # remove intermediates
...
#   select(condition, entry, meta_surfaceome) %>%
...
plot_data <- super_volcano_candi_prot_L %>%
  filter(log2FC == Inf) %>%
  select(comparison, entry, meta_surfaceome) %>%
...
svcp_L_sigupuni <- super_volcano_candi_prot_L %>% 
  ...
  select("comparison", "entry", "entry_name", "meta_surfaceome", "replicate_ids_left_right") %>%
  left_join(proteome %>% select("entry", "gene_ontology_i_ds"), #  add go term info
            by = "entry") %>%
...
sums_by_condition <- svcp_L_sigupuni %>%    # category summary per condition & join with annotation
  ...
  select(-subcategory_order, -subcategory) %>%
...
data <- svcp_L_sigupuni %>%
  select(meta_surfaceome, conditions_listed) %>%
  table()
...
prot_level_data_sigupuni <- data_level_prot %>% 
  ...
  select(condition, bio_replicate, con_rep, entry, entry_name, protein_name, imputed_prot_intensity_log2, features_left_right_overall, replicate_ids_left_right)

library(tidyverse)
library(pheatmap)
# Prepare the data for the heatmap
heatmap_data <- prot_level_data_sigupuni %>%
  select(con_rep, entry_name, imputed_prot_intensity_log2) %>%

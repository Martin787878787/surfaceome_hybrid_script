...
go_gost <- function(query_list, max_term_size, set, go_source) {
  ...
  result <- gost(query = gconvert(query_list, #setdiff(data_CSC_w_ENSG_table$Act_0, data_CSC_w_ENSG_table$Act_2h),  # this works best, uniprot identifiers worst, primary_gene_names good
                 ...
                 numeric_ns = "",
                 # !!!
                 sources = go_source, # <<<<<< !!!! When sources is set to NULL (the default), the analysis includes all Gene Ontology branches (GO:BP, GO:MF, GO:CC), KEGG pathways, Reactome (REAC), transcription factors (TF), miRNA targets (MIRNA), Human Phenotype Ontology (HP), CORUM protein complexes, WikiPathways (WP), and tissue-specific annotations (HPA).
                 # !!!
                 as_short_link = FALSE,
                 highlight = TRUE)
  ...
  result_result  <- result$result  %>% 
    ...
    dplyr::select(term_name, comparison, term_size, p_value, recall, intersection, evidence_codes) %>%
    arrange(desc(recall)) 
  ...
  # translate to gene and uniprot entry based on intersection column and function defined above
  result_result <- apply_mapping(result_result, mapping, column_to_retrieve = "external_gene_name", new_column_name = "intersection_gene")  # specify which columns to translate
  result_result <- apply_mapping(result_result, mapping, column_to_retrieve = "uniprot_gn_id"     , new_column_name = "intersection_up")    # specify which columns to translate

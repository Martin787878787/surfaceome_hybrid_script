...
diff_data_input  <-  read.csv("/Users/mgesell/Desktop/currentR/2025-01__local_reanalysis_paper_candi_experiements/v31_LUX_FP20_HoxHoxox_semi_6aa__4ss/_output_1-2_ludo_adjp_0.7string_uniqueImpute_shs2.22/_super_volcano_candi_prot_L.csv", header = TRUE)
condition_levels  =  c("meta") #, "nCD4_TCR", "nnCD4_TCR", "nCD8_TCR", "nnCD8_TCR")

for (condi in condition_levels) {
  message(paste0(" ___________________ running condition = ", condi, " ____________________________________________________________________________ "))
  # define changes either meta or individual conditions
  if ( condi == "meta") {
    changes <- diff_data_input %>% 
      dplyr::filter(log2FC >= 1, adj_pvalue <= 0.05) %>%    # no condition filtering
      pull(entry) %>% 
      unique()  
    } else if ( condi != "meta") {
    changes <- diff_data_input %>% 
      dplyr::filter(condition == condi,                    # filter for condition 
                    log2FC >= 1, adj_pvalue <= 0.05) %>% 
      pull(entry) %>% 
      unique()
   }

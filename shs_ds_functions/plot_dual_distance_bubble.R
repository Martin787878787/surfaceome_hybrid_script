...
plot_dual_distance_bubble <- function(data, group_filter, min_recall, max_p_value, 
  ...
  data_ordered <- data_ordered %>%
    mutate(border_width = case_when(recall == 1 ~ 1.5, TRUE ~ 0.5)) # highlight 100% recall values in plot
  min_bubble_size = if_else(min(data_ordered$recall, na.rm = TRUE) < 0.33, 0, 0.33-0.13)
  ...
  data_output <- data_ordered %>%
    arrange(desc(recall)) %>%
    distinct(!!sym(term_column), .keep_all = TRUE) #%>%
  #arrange(desc(row_number()))  # inverts order of df so easy to compare output table with plot
  
  result <- list(plot, data_output)
  return(result) 
...
#                  labels = label_number(accuracy = 0.01)      ) +  # ensures two digits

...
complex_corum <- function(query_list, set) {
  ...
  for (entry in query_list) {
    ...
    corum$matches[matches] <- ifelse(corum$matches[matches] == "", 
                                     entry, 
                                     paste(corum$matches[matches], entry, sep = ", "))
  }
  
  corum_query_subset <- corum %>%
    dplyr::select(complex_name, comment_complex, subunits_uniprot_id, matches, complex_size, complex_counter_cor) %>%

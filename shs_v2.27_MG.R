...
# 2.26  2025-06-23    mapping CSC signatures to protein sequence for comparison predicted vs. experimentally shown vs. novel
# 2.27  2025-06-27    adding csc-site novel/validated/known annotation based on uniprot
...
input_directory       = c("/Users/mgesell/Desktop/currentR/2025-01__local_reanalysis_paper_candi_experiements/AAV_CSC_v2_cell_lines")
...
##
experiment_type       = "peptideCSC"     #  choose between "gLUX" "Lysate_ProtCSC_LUX_LUXCSC", "peptideCSC", and "hybrid_CSC_LUX" (last one can be used to require both deamidation and LUX ox on same peptides)
...
#
experiment_conditions = c("HeLa", "HEK" , "HepG2", "K562") # order needs to match experiment_references order !!!    variable determines loop:   for (i in 1:length(experiment_conditions)) ... condition = experiment_conditions[i]
experiment_references = c("HEK" , "HeLa", "HeLa" , "HeLa") # order needs to match experiment_conditions order !!!    variable determines loop:   for (i in 1:length(experiment_references)) ... condition = experiment_references[i]
#
...
if (data_type == "DDA") { 
  ...
  data_raw  <- data_raw_ %>% 
    ...
    dplyr::rename(PSM_id = protein_name)
  ...
  data_raw  <- data_raw_ %>% 
    ...
    filter(!(run %in% paste(filter_exclude_files))) %>% # Exclude files based on filter_exclude_files
    dplyr::rename(PSM_id = all_mapped_proteins, entry = protein_group, peptide_sequence_mod= modified_sequence,stripped_peptide_sequence= stripped_sequence , raw_prec_intensity=precursor_quantity) %>%    ### introduce more meaningfull column names to avoid confusion later on
...
data_raw_up$stripped_peptide_sequence <- gsub("\\[0.9840\\]|\\[57.0215\\]|\\[15.9949\\]|n\\[42.0106\\]|\\[31.9898\\]|\\[13.9793\\]",
                                              "", data_raw_up$peptide_sequence_mod)  # create stripped pep seq column
data_raw_up$semi_stripped_peptide_sequence <- gsub("\\[57.0215\\]|\\[15.9949\\]|n\\[42.0106\\]|\\[31.9898\\]|\\[13.9793\\]", 
                                                   "", data_raw_up$peptide_sequence_mod)
...
data_raw_up_surf <- data_raw_up %>%
  ...
  dplyr::rename(meta_surfaceome = cspa_2015surfy_2018tcsa_2021cd_antigen_veneer_proteome_high) # CSPA_CD_orDouble_Surfy_TCSA_VeneerHigh (= meta_surfaceom_v3.0)   cspa_2015surfy_2018tcsa_2021cd_antigen_veneer_proteome_high = (meta_surfaceome_v1.0)
...
if (experiment_type == "peptideCSC") {
  boxplot(data_raw_up_surf_gly %>% dplyr::filter(csc_signature_psm == "yes") %>% dplyr::select(sequence_extended, csc_signature_psm_n) %>% distinct() %>% pull(csc_signature_psm_n))
}
data_raw_up_surf_gly <- data_raw_up_surf_gly %>% 
...
# annotate AA postion of CSC signature
map_ptm_positions <- function(peptide, protein_start) {
  peptide <- as.character(peptide)
  n <- nchar(peptide)
  aa_count <- 0
  mod_positions <- integer(0)
  i <- 1
  while (i <= n) {
    char_i <- substr(peptide, i, i)
    if (grepl("[A-Z]", char_i)) {
      aa_count <- aa_count + 1
      # Check for modification block
      if (i + 1 <= n && substr(peptide, i + 1, i + 1) == "[") {
        # Record current amino acid position
        mod_positions <- c(mod_positions, aa_count)
        # Skip modification block
        j <- i + 1
        while (j <= n && substr(peptide, j, j) != "]") j <- j + 1
        if (j <= n) i <- j  # jump to closing bracket
      }
    }
    i <- i + 1
  }
  if (length(mod_positions) == 0) return(NA)
  # Calculate absolute positions: protein_start + (aa_count - 1)
  abs_positions <- protein_start + mod_positions - 1
  paste0(abs_positions, "-Ng", collapse = " ")
}

# Apply to dataframe
data_raw_up_surf_gly <- data_raw_up_surf_gly %>%
  mutate(ng_sites = mapply(map_ptm_positions, 
                           data_raw_up_surf_gly$semi_stripped_peptide_sequence, # only N[0.9840] remains (workaround because otherwise also other ptms assigend ...)
                           data_raw_up_surf_gly$start),                         # attention donßt use protein_start i think that is FP reportet RT,
         ng_sites =  case_when(csc_signature_psm == "yes" ~ ng_sites, TRUE ~ NA)) # for all non CSC_psm overwrite IDs
...
write.csv(data_raw_up_surf_gly_nZ,  file = paste0(output_directory, "/data_raw_up_surf_gly_nZ.csv"))
write.csv(data_raw_up_surf_nZ_glyF, file = paste0(output_directory, "/data_raw_glyco_nZ_gyIFcsc.csv"))
...
if (experiment_type == "peptideCSC") {
  ...
  Fig2.3.1_i <- Ng_summary %>%
    ...
    labs(title = "N-glyco sites",  x = "Overall",  y     = "Glyco site count",    fill  = "Category"  ) +
...
if (data_processing == "protti"){    #-------- Protti -------- Protti -------- Protti -------- Protti -------- Protti -------- Protti -------- Protti -------- Protti 
  ...
  if (protti_normalization == "median") { 
    data_prec_norm <- data_raw_pre_filtered %>% 
      ...
      dplyr::rename(normalised_prec_intensity_log2 = normalised_intensity_log2) %>% # to avoid confusion with protein quantitiy that is created later on
  ...
  data_prec_norm_feature_filtered <- data_prec_norm_feature_filtered %>%  
    dplyr::select(precursor, global_median_log2_feature_intensity) %>%
    distinct(precursor, global_median_log2_feature_intensity, .keep_all = TRUE) %>%
    mutate(rank_feature_global = dense_rank(desc(global_median_log2_feature_intensity))) %>%
    right_join(data_prec_norm_feature_filtered, 
               by = c("precursor", "global_median_log2_feature_intensity")) %>%
    arrange(desc(global_median_log2_feature_intensity))
  # rank_feature_condition
  data_prec_norm_feature_filtered <- data_prec_norm_feature_filtered %>%
    group_by(precursor, condition) %>%
    mutate(rank_feature_condition = dense_rank(desc(condition_median_log2_feature_intensity))) %>%
    ungroup() %>%
    arrange(condition, desc(condition_median_log2_feature_intensity))
  ...
  data_prot <-  calculate_protein_abundance(  
    ...
    dplyr::rename(normalised_protein_intensity_log2 = normalised_prec_intensity_log2)  # needs to be removed if "filter(precursor == "protein_intensity")" line is deleted
  ...
  if(protti_imputation == "no_imputation") {
    ...
    ) %>%  dplyr::rename(imputed_prot_intensity_log2 = imputed_intensity)
  }
...
for (comp_counter in 1:length(loop_frame)) {   # ----------------------- LOOOOOOOP --------------------- LOOOOOOOP ------------------------------ LOOOOOOOP ------------------- LOOOOOOOP -------------------
  ...
  write.csv(super_volcano_data[order(super_volcano_data$left_right, decreasing = TRUE),] %>% dplyr::rename(Protein = entry, Label = comparison),
  ...
  scatter_plot_data <- data_level_prot[data_level_prot$comparison == paste0(usedConditions[1], "_vs_", usedConditions[2]), c("condition", "entry", "entry_name", "protein_name", "condition_median_imp_log2_intensity", 
    ...
    dplyr::rename(meta_surfaceome = cspa_2015surfy_2018tcsa_2021cd_antigen_veneer_proteome_high)  %>%
...
sums_by_condition <- svcp_L_sigupuni %>%    # category summary per condition & join with annotation
  ...
  dplyr::rename(subcategory = subcategory.y) %>%
  mutate(

...
# 2.3  2025-06-18 repeat and ggf extend GO analysis for Romain
...
script_version = "_shs-ds2.3"  # version


# Load libraries  ----------------------------------------------------------------------------------------------------------
# Load libraries  ----------------------------------------------------------------------------------------------------------
library(tidyverse)    # Data wrangling, general functions, plotting, piping, list handling (loads dplyr, tidyr, ggplot2, magrittr, purrr, etc.)
library(viridis)
...
directory_root        = c("/Users/mgesell/Desktop/currentR/2025-01__local_reanalysis_paper_candi_experiements/v31_shs-ds")
...
# Romain GO reanalysis LUX ____________________________________________________________________________________
enrichment_table <- data.frame()  # Initialize
counter_go_loop = 1
sets = c("overall", "nCD4", "nnCD4", "nCD8", "nnCD8")

for (query_lists in c("overall", "nCD4_TCR_vs_nCD4_Iso", "nnCD4_TCR_vs_nnCD4_Iso", "nCD8_TCR_vs_nCD8_Iso", "nnCD8_TCR_vs_nnCD8_Iso")) {
  for (analysis_types in c("GO", "GO_slim", "hallmark")) {
    cat("_____ for", sets[counter_go_loop], "running", analysis_types, "_____\n")
    
    if(query_lists == "overall") {
      query = unique(data_LUX_prot_diff_v31_meta_full %>% filter(                           log2FC>1, adj_pvalue <= 0.05) %>% pull(entry))
    } else {
      query = unique(data_LUX_prot_diff_v31_meta_full %>% filter(comparison == query_lists, log2FC>1, adj_pvalue <= 0.05) %>% pull(entry))
    }

    enrich_res <- perform_enrichment_uniprot(query_list = query,  analysis_type = analysis_types, set = sets[counter_go_loop])  # analysis_type = c("GO", "GO_slim", "hallmark"
    
    enrichment_table <- rbind(enrichment_table, enrich_res@result)
  }
  counter_go_loop <- counter_go_loop + 1
}
rm(counter_go_loop) 

enrichment_table_filtered <- enrichment_table %>%
  filter(pvalue <= 0.05, qvalue <= 0.2) %>%
  separate(GeneRatio, into = c("n_detected" , "n_query"     ), sep = "/", convert = TRUE) %>%
  separate(BgRatio  , into = c("n_annotated", "n_background"), sep = "/", convert = TRUE) %>%
  mutate(recall = n_detected/n_annotated) %>%
  dplyr::rename(p_value = pvalue, term_name = Description)

length(unique(enrichment_table_filtered$term_name))
View(enrichment_table_filtered)
write.csv(enrichment_table_filtered, paste0(directory_output, "/_GO-GOslim-Hallmark_enrichment.csv") , row.names = FALSE)

# plotting ------
common_params <- list( # Define common plot parameters
  data = enrichment_table_filtered,  min_recall = 1,    max_p_value = 0.05,    grouping = "comparison",    term_column = "term_name",    distance_column = "recall",    distance_method = "euclidean", # distance parameters
  x_var = "comparison",    y_var = "term_name",    size_var = "recall",    fill_var = "p_value",    title_var = "GO Term Enrichment" # plot parameters
)
group_filters <- list( # Define different group filters
  NULL,
...
# Romain GO reanalysis CSC ____________________________________________________________________________________
enrichment_table <- data.frame()  # Initialize
counter_go_loop = 1
sets = c("overall", "nCD4", "nnCD4", "nCD8", "nnCD8")

for (query_lists in c("overall", "nCD4", "nnCD4", "nCD8", "nnCD8")) {
  for (analysis_types in c("GO", "GO_slim", "hallmark")) {
    cat("_____ for", sets[counter_go_loop], "running", analysis_types, "_____\n")
    
    if(query_lists == "overall") {
      query = unique(data_CSC_prot_v31 %>% filter(                        ) %>% pull(entry))
    } else {
      query = unique(data_CSC_prot_v31 %>% filter(condition == query_lists) %>% pull(entry))
    }
...
length(unique(enrichment_table_filtered$term_name))
View(enrichment_table_filtered)
write.csv(enrichment_table_filtered, paste0(directory_output, "/_GO-GOslim-Hallmark_enrichment_CSC.csv") , row.names = FALSE)
...
common_params <- list( # Define common plot parameters
  data = gost_CSC,
  min_recall = 1,  max_p_value = 0.05,  grouping = "comparison",  term_column = "term_name", distance_column = "recall",  distance_method = "euclidean", # distance parameters
...
common_params <- list( # Define common plot parameters
  data = comp_cp_LUX,  min_recall = 1,    max_p_value = NULL,    grouping = "comparison",    term_column = "recommended_name",    distance_column = "complex_recall_cp",    distance_method = "euclidean", # distance parameters
...
# plotting ------
common_params <- list( # Define common plot parameters
  data = comp_cp_CSC,  min_recall = 0.66,    max_p_value = NULL,    grouping = "comparison",    term_column = "recommended_name",    distance_column = "complex_recall_cp",    distance_method = "euclidean", # distance parameters
  x_var = "comparison",    y_var = "recommended_name",    size_var = "complex_recall_cp",  fill_var = NULL,    title_var = "Complex Portal" # plot parameters
)
group_filters <- list( # Define different group filters
  NULL,
...
common_params <- list( # Define common plot parameters
  data = comp_cor_LUX,  min_recall = 1,    max_p_value = NULL,    grouping = "comparison",    term_column = "complex_name_cor",    distance_column = "complex_recall_cor",    distance_method = "euclidean", # distance parameters
...
# plotting ------
common_params <- list( # Define common plot parameters
  data = comp_corum_CSC,  min_recall = 1,    max_p_value = NULL,    grouping = "comparison",    term_column = "complex_name_cor",    distance_column = "complex_recall_cor",    distance_method = "euclidean", # distance parameters
  x_var = "comparison",    y_var = "complex_name_cor",    size_var = "complex_recall_cor",    fill_var = NULL,    title_var = "Corum Complexes" # plot parameters
)
group_filters <- list( # Define different group filters
  NULL,
...
LUX_CSC_map_wide <- LUX_CSC_map %>%
  ...
  left_join(proteome %>% dplyr::select(entry, entry_name), by = "entry")
